...
La gráfica muestra dos “picos” fuera de la tendencia observada: el miércoles 01 de abril por la noche y el domingo 06 de abril por la tarde. El primero corresponde con el momento en el que el gobierno formalizó la jornada de sana distancia —es decir, pidió a las y los ciudadanos quedarse en casa— hasta el 30 de abril. El segundo coincide en fecha y horario con el informe del presidente de la República en el que dio a conocer su plan de acción económica para enfrentar la pandemia.
...
if(is_html_output()) {
    plotly <- plotly::ggplotly(ur+theme(axis.text.x = element_text(size=8)), 
                             tooltip = c("x", "y", "size"),
                             dynamicTicks = TRUE
  )%>%
    plotly::rangeslider() %>%
...
if(is_html_output()) {
    plotly <- plotly::ggplotly(ur+theme(axis.text.x = element_text(size=8)),tooltip = c("label", "y", "text"))%>%
    plotly::layout(title = "",
                   annotations = list(x = 1, y = 0, 
                                      text = caption, 
                                      showarrow = F, 
